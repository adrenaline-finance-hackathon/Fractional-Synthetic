...
describe("Stable Collateral Reserve", async () => {
  // * MAIN CONTRACTS
  let collateralReserve, pidController, reserveTracker, kusdPool;

  // * ORACLES
  let dopxOracle, kusdOracle, usdcOracle;

  // * PAIR CONTRACTS
  let pairShareUsdc, pairShareBusd;

  // * ERC20 COIN CONTRACTS
  let usdc, busd, dopx, kusd;
  ...
  const deployAll = async () => {
    ...
    // * deploy KUSD
    [kusd] = await deployProxy("KUSD", [owner.address, "Kelly USD", "KELLY"]);

    // * deploy kusd oracle
    kusdOracle = await deployContract("MockPairOracle", [ONE_ETHER]);
    ...
    // await usdc.transfer(minter.address, MILLION_ETHER);
    // await usdc.mint(owner.address, MILLION_ETHER);
    ...
    // deploy StableCollateralReserve
  ...
  const mockTGSV = async (tgsv) => {
    ...
    await kusd.connect(minter).mint(collateralReserve.address, tgsv);
  };
  ...
  const allBalanceOf = async (_user) => {
    const [usdcBalance, doppleXBalance, kusdBalance] = await Promise.all([
      usdc.balanceOf(_user.address),
      dopx.balanceOf(_user.address),
      kusd.balanceOf(_user.address),
    ]);
    return {
      usdc: fromWei(usdcBalance),
      dopx: fromWei(doppleXBalance),
      kusd: fromWei(kusdBalance),
    };
  };
  ...
  describe("# Check Numbers", () => {
    ...
    describe("## `totalGlobalSynthValue`", () => {
      it("should be zero if no synth totalSupply", async () => {
        // add Synth
        await collateralReserve.addSynth(kusd.address);
        expect(await kusd.balanceOf(collateralReserve.address)).to.eq(BIG_ZERO);
      ...
      it("should mock totalGlobalSynthValue to 1M", async () => {
        ...
        // set token cap
        await kusd.connect(owner).setTokenCap(toWei("1000000"));
      ...
      it("should mock totalGlobalSynthValue to 100M", async () => {
        ...
        // set token cap
        await kusd.connect(owner).setTokenCap(toWei("100000000"));
      ...
      it("should be 10k with 1 synth on synth 0.1$ totalSupply 100k", async () => {
        ...
        // mock synth oracles
        await setPrice(kusdOracle, 0.1);
        // add Synth
        await collateralReserve.addSynth(kusd.address);

        // set token cap
        await kusd.connect(owner).setTokenCap(toWei("100000"));
    ...
    describe("## `getMaxBuybackShare`", () => {
      ...
      it("should be getMaxBuybackShare with $10/usdc, $1/dopx, $1/kusd", async () => {
        // * ECR 1000%
        await mockECR(toWei("100"), toWei("10"));
      ...
      it("should be getMaxBuybackShare with $1/usdc, $1/dopx, $0.1/kusd", async () => {
        // * ECR 1000%
        await mockECR(toWei("1000"), toWei("10"));
      ...
      it("should be getMaxBuybackShare with $1/usdc, $0.1/dopx, $1/kusd", async () => {
        // * ECR 1000%
        await mockECR(toWei("1000"), toWei("10"));
        await setPrice(usdcOracle, "1");
    ...
    describe("## `getECR`", () => {
      ...
      it("should be 100% then decrease to 80% after KUSD prices goes up to 1.25", async () => {
        // * ECR 100%
        const _gcv = toWei("100");
        const _tgsv = toWei("100");
        await mockECR(_gcv, _tgsv);
        ...
        // * increase price
        await setPrice(kusdOracle, 1.25);

        expect(await collateralReserve.getECR()).to.eq(toWei(toPercent("80")));
      });

      it("should be 100% then increase to 200% after KUSD prices goes down to 0.5", async () => {
        // * ECR 100%
        const _gcv = toWei("100");
        const _tgsv = toWei("100");
        await mockECR(_gcv, _tgsv);
  ...
  describe("# Vault", () => {
    ...
    it("should call rebalanceVault with correct amount after collateral increase", async () => {
      await usdc.mint(collateralReserve.address, toWei("100"));
    ...
    it("should throw error if buyBack more than token has", async () => {
      ...
      await collateralReserve
        .connect(user1)
        .buyBackShare(toWei("20"), BIG_ZERO, usdc.address);
      ...
      expect(await treasuryVaultVenus.vaultBalance()).to.be.equal(toWei("56"));
    });
    ...
    it("should get globalCollateralValue after enterVault and reblance then recall which no change", async () => {
      const [owner] = await ethers.getSigners();
      // mint
      await usdc.connect(owner).mint(collateralReserve.address, toWei("100"));
  ...
  describe("# Access control and Set functions", () => {
    it("only MAINTAINER is able to 'addPool'", async () => {
      ...
      await expect(
        collateralReserve.connect(anotherAccount).addPool(kusdPool.address)
      ...
      await expect(collateralReserve.connect(owner).addPool(kusdPool.address))
        .to.emit(collateralReserve, "PoolAdded")
        .withArgs(kusdPool.address);
      expect(await collateralReserve.synthPoolExist(kusdPool.address)).to.be
        .true;
      expect(await collateralReserve.synthPoolArray(BIG_ZERO)).to.eq(
        kusdPool.address
      );
      // POOL = pool
      expect(await collateralReserve.hasRole(POOL, kusdPool.address)).to.be
        .true;
      expect(await collateralReserve.getRoleMemberCount(POOL)).to.eq(BIG_ONE);

      await expect(
        collateralReserve.connect(owner).addPool(kusdPool.address)
    ...
    it("only MAINTAINER is able to 'removePool'", async () => {
      ...
      await expect(
        collateralReserve.connect(anotherAccount).removePool(kusdPool.address)
      ).to.be.revertedWith("Sender is not a maintainer");
      ...
      // remove pool
      await expect(
        collateralReserve.connect(owner).removePool(kusdPool.address)
      )
    ...
    it("only MAINTAINER is able to 'addCollateralAddress'", async () => {
      ...
      await expect(
        collateralReserve
          .connect(anotherAccount)
          .addCollateralAddress(kusd.address, kusdOracle.address)
      ).to.be.revertedWith("Sender is not a maintainer");

      await expect(
        collateralReserve
          .connect(owner)
          .addCollateralAddress(ADDRESS_ZERO, kusdOracle.address)
      ).to.be.revertedWith("Zero address detected");
      ...
      //add oracle
      await collateralReserve.connect(owner).addOracle(kusdOracle.address);
      expect(await collateralReserve.oracleExist(kusdOracle.address)).to.be
        .true;
      ...
      await expect(
        ...
        .withArgs(kusd.address);
      expect(await collateralReserve.collateralAddress(kusd.address)).to.be
        .true;
      expect(await collateralReserve.collateralAddressArray(BIG_ONE)).to.eq(
        kusd.address
      );

      expect(await collateralReserve.oracleOf(kusd.address)).to.eq(
        kusdOracle.address
      );
    ...
    it("only MAINTAINER is able to add oracles", async () => {
      ...
      await expect(
        collateralReserve.connect(owner).addOracle(kusdOracle.address)
      )
        .to.emit(collateralReserve, "AddOracle")
        .withArgs(kusdOracle.address);
      expect(await collateralReserve.oracleExist(kusdOracle.address)).to.be
        .true;
      expect(await collateralReserve.oracleArray(BIG_ONE)).to.eq(
        kusdOracle.address
    ...
    it("only MAINTAINER is able to 'addSynth'", async () => {
      ...
      expect(await collateralReserve.hasRole(MAINTAINER, owner.address)).to.be
        .true;
      await expect(
        collateralReserve.connect(account1).addSynth(kusd.address)
      ).to.be.revertedWith("Sender is not a maintainer");

      // add synth
      await expect(collateralReserve.connect(owner).addSynth(kusd.address))
        .to.emit(collateralReserve, "AddSynthToken")
        .withArgs(kusd.address);
      expect(await collateralReserve.synthExists(kusd.address)).to.be.true;
      expect(await collateralReserve.synthArray(BIG_ZERO)).to.eq(kusd.address);

      await expect(
        collateralReserve.connect(owner).addSynth(kusd.address)
      ).to.be.revertedWith("Address already exists");
      await expect(
