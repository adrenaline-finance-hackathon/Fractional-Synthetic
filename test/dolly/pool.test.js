...
describe("Convert Dopple", function () {
  ...
  beforeEach(async () => {
    ...
    const SuperDop = await ethers.getContractFactory("SuperDopple");
    superDop = await SuperDop.deploy();
    ...
    pairBusdSuperDop = await MockPair.deploy(
      superDop.address,
      busd.address,
      ONE,
    ...
    iPairBusdSuperDop = await ethers.getContractAt(
      "IUniswapV2Pair",
      pairBusdSuperDop.address
    );
  ...
  it("Shall have reserve in mock pair", async function () {
    const [reserve0, reserve1] = await pairBusdSuperDop.getReserves();
    expect(reserve0).to.equals(utils.parseEther("1"));
    expect(reserve1).to.equals(utils.parseEther("0.5"));
  });

  it("Shall get lastest price of sDOP from oracle", async function () {
  ...
  it("Shall set lastest price of sDOP to 0.6", async function () {
    const token = busd.address;
    const amountIn = utils.parseEther("1");
